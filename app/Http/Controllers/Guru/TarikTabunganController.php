...
class TarikTabunganController extends Controller
{
    public function data(Request $request)
    {
        ...
        $data = DB::table('transaksi_tabungans')
            ...
            ->where('transaksi_tabungans.jenis_transaksi', 'tarik')
        ...
        return datatables($data)
            ...
            ->addColumn('aksi', function ($row) {
                // <button onclick="downloadPDF(`' . route('tarik.downloadPDF', $row->id) . '`)" class="btn btn-primary btn-sm">
                //     <i class="fa fa-file-pdf"></i> Download
                // </button>
                return '
                    <button onclick="deleteData(`' . route('tarik.destroy', $row->id) . '`, `' . $row->nama_lengkap . '`, `' . $row->tanggal_transaksi . '`, `' . $row->jumlah . '`)" class="btn btn-danger btn-sm">
    ...
    {
        return view('guru.tabungan.tarik.index');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($siswa->saldo <  $jumlah) {
            return response()->json([
                'status' => 'error',
                'message' => 'Saldo tidak cukup untuk melakukan tarik tunai',
            ], 400);
        }
        ...
        $prefix = 'INV';
        $date = date('Ymd');
        $last = TransaksiTabungan::where('jenis_transaksi', $request->jenis_transaksi)
        ...
        return response()->json([
            'status' => 'success',
            'message' => 'Data berhasil disimpan dan saldo diperbarui',
        ], 200);
    }
    ...
    public function PDF1($id)
    {
        ...
        return view('guru.tabungan.tarik.cetak_bukti_tarik', compact('siswa', 'guru', 'tabungan'));
    }
    ...
    public function PDF($id)
    {
        ...
        // Generate PDF
        $pdf = Pdf::loadView('guru.tabungan.tarik.cetak_bukti_tarik', compact('siswa', 'guru', 'tabungan'));

        // Atur ukuran kertas dan orientasi (F4)
        $pdf->setPaper('A4', 'landscape');
        return $pdf->stream('tanda_bukti_tarik_tunai.pdf');

        // Download PDF
        // return $pdf->download('tanda_bukti_tarik_tunai.pdf');
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'status' => 'success',
            'message' => 'Data berhasil dihapus dan saldo dikembalikan'
        ], 200);
    }
}
